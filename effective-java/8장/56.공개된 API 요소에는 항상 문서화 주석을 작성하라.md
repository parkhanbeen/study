...
* 상속용으로 설계된 클래스(아이템 19)의 메서드가 아니라면 (그 메서드가 어떻게 동작하는지가 아니라) 무엇을 하는지를 기술해야 한다.
...
* 아이템 15에서 이야기 했듯, 클래스를 상속용으로 설계할 때는 자기사용 패턴(self-use pattern)에 대해서도 문서에 남겨 다른
...
* 자바 9부터 지원하는 모듈 시스템(아이템 15)도 이와 비슷한다.
